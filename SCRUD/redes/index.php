...
<div class='container'>
    <div class='row'>
        <h2>Mantenimiento de Redes Sociales</h2>
    </div>
    <div class='row'>
        ...
        <table class='table table-striped table-bordered table-hover'>
            ...
            <?php
                require("db.php");
                $sql = "SELECT id_red, nombre FROM redes_sociales ORDER BY id_red DESC";
                $data = "";
                foreach($PDO->query($sql) as $row) {
                    $data .= "<tr>";
                    $data .= "<td>$row[id_red]</td>";
                    $data .= "<td>$row[nombre] </td>";
                    $data .= "<td>";
                     $data .= "<a class='btn btn-xs btn-info' href='read.php?id=$row[id_red]'>Consultar</a>&nbsp;";
                    $data .= "<a class='btn btn-xs btn-primary' href='update.php?id=$row[id_red]'>Actualizar</a>&nbsp;";
                    $data .= "<a class='btn btn-xs btn-danger' href='delete.php?id=$row[id_red]'>Eliminar</a>";
                    $data .= "</td>";
                    $data .= "</tr>";
                }
                print($data);
