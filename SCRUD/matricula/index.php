...
<div class='container'>
    <div class='row'>
        <h2>Mantenimiento de Matricula</h2>
    </div>
    <div class='row'>
        ...
        <table class='table table-striped table-bordered table-hover'>
            ...
            <?php
                require("db.php");
                $sql = "SELECT id_ma, precio, descripcion FROM matricula ORDER BY id_ma DESC";
                $data = "";
                foreach($PDO->query($sql) as $row) {
                    $data .= "<tr>";
                    $data .= "<td>$row[id_ma]</td>";
                    $data .= "<td>$row[precio] </td>";
                    $data .= "<td>$row[descripcion] </td>";
                    $data .= "<td>";
                    $data .= "<a class='btn btn-xs btn-primary' href='update.php?id=$row[id_ma]'>Actualizar</a>&nbsp;";
                    $data .= "<a class='btn btn-xs btn-danger' href='delete.php?id=$row[id_ma]'>Eliminar</a>";
                    $data .= "</td>";
                    $data .= "</tr>";
                }
                print($data);
