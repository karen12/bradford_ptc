...
<div class='container'>
    ...
    <div class='row'>
        ...
        <table class='table table-striped table-bordered table-hover'>
            ...
            <?php
                require("db.php");
                $sql = "SELECT id_articulo, nombre_art FROM utiles ORDER BY id_articulo DESC";
                $data = "";
                foreach($PDO->query($sql) as $row) {
                    $data .= "<tr>";
                    $data .= "<td>$row[id_articulo]</td>";
                    $data .= "<td>$row[nombre_art] </td>";
                    $data .= "<td>";
                    $data .= "<a class='btn btn-xs btn-info' href='read.php?id=$row[id_articulo]'>Consultar</a>&nbsp;";
                    $data .= "<a class='btn btn-xs btn-primary' href='update.php?id=$row[id_articulo]'>Actualizar</a>&nbsp;";
                    $data .= "<a class='btn btn-xs btn-danger' href='delete.php?id=$row[id_articulo]'>Eliminar</a>";
                    $data .= "</td>";
                    $data .= "</tr>";
                }
                print($data);
