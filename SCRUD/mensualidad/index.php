...
<div class='container'>
    <div class='row'>
        <h2>Mantenimiento de Mensualidad</h2>
    </div>
    <div class='row'>
        ...
        <table class='table table-striped table-bordered table-hover'>
            ...
            <?php
                require("db.php");
                $sql = "SELECT id_men, nivel, precio FROM mensualidad ORDER BY id_men DESC";
                $data = "";
                foreach($PDO->query($sql) as $row) {
                    $data .= "<tr>";
                    $data .= "<td>$row[id_men]</td>";
                    $data .= "<td>$row[nivel] </td>";
                    $data .= "<td>$row[precio] </td>";
                    $data .= "<td>";
                    $data .= "<a class='btn btn-xs btn-primary' href='update.php?id=$row[id_men]'>Actualizar</a>&nbsp;";
                    $data .= "<a class='btn btn-xs btn-danger' href='delete.php?id=$row[id_men]'>Eliminar</a>";
                    $data .= "</td>";
                    $data .= "</tr>";
                }
                print($data);
