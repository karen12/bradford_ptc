...
<div class='container'>
    ...
    <div class='row'>
        ...
        <table class='table table-striped table-bordered table-hover'>
            ...
            <?php
                require("db.php");
                $sql = "SELECT id_docente,nombre,apellido,dui FROM docente ORDER BY id_docente DESC";
                $data = "";
                foreach($PDO->query($sql) as $row) {
                    $data .= "<tr>";
                    $data .= "<td>$row[id_docente]</td>";
                    $data .= "<td>$row[nombre] </td>";
                    $data .= "<td>$row[apellido]</td>";
                    $data .= "<td>$row[dui] </td>";
                    $data .= "<td>";
                    $data .= "<a class='btn btn-xs btn-info' href='read.php?id=$row[id_docente]'>Consultar</a>&nbsp;";
                    $data .= "<a class='btn btn-xs btn-primary' href='update.php?id=$row[id_docente]'>Actualizar</a>&nbsp;";
                    $data .= "<a class='btn btn-xs btn-danger' href='delete.php?id=$row[id_docente]'>Eliminar</a>";
                    $data .= "</td>";
                    $data .= "</tr>";
                }
                print($data);
